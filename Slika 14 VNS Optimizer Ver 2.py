...
class NeuroOptimizerApp(tk.Tk):
    def __init__(self):
        ...
        # Populacija će biti inicijalizovana tokom optimizacije
    ...
    def chaotic_initialization(self, pop_size, dim, c0=0.7):
        # Inicijalizacija populacije korišćenjem logističke mape
    ...
    def evaluate_fitness(self, agent):
        # Primer funkcije za evaluaciju – suma komponenti (što znači da veća suma označava bolje rešenje)
        # U praksi zameniti odgovarajućom funkcijom koja meri performanse rešenja.
        return np.sum(agent)

    def vns_search(self, agent):
        # Simulacija VNS mehanizma – mala lokalna pretraga (shaking + protokol poboljšanja)
        perturbation = np.random.uniform(-0.1, 0.1, size=agent.shape)
        new_agent = agent + perturbation
        return new_agent

    def fa_search(self, agent, best_agent, fa_params):
        # Simulacija Firefly algoritma: ažuriraj agenta prema najboljem rešenju uz dodatak slučajnosti
        ...
        new_agent = agent + attractiveness * (best_agent - agent) + alpha * (np.random.rand(*agent.shape) - 0.5)
        return new_agent
    ...
    def _run_optimization(self):
        ...
        levy_e = float(self.vns_params["Levy E"].get())  # trenutno se ne koristi, ali ostaje za potencijalno proširenje
        ...
        chaotic_ES = 0.7  # inicijalna haotična vrednost
        ...
        for t in range(max_iter):
            ...
            # 5: Ponovo izračunaj ES (haotični parametar preko logističke mape)
            chaotic_ES = 4 * chaotic_ES * (1 - chaotic_ES)

            # 6: Za svakog agenta u populaciji radi – ažuriranje rešenja
            for i in range(pop_size):
                current_agent = self.population[i]
                if t > theta:
                    # 7: Ako je t > θ, koristi se VNS mehanizam pretrage
                    new_agent = self.vns_search(current_agent)
                else:
                    # 9: Inače, generiši nasumičnu vrednost R
                    R = np.random.uniform(0, 1)
                    if R < 0.5:
                        # 11: Ako je R < 0.5, koristi se VNS mehanizam pretrage
                        new_agent = self.vns_search(current_agent)
                    else:
                        # 14: Inače, koristi se FA mehanizam pretrage
                        new_agent = self.fa_search(current_agent, best_agent, fa_params)
                # Opcionalno – ažuriranje najboljeg agenta pomoću haotičnog elitnog učenja:
                # X' = X + rand*(chaotic_ES - 1)
                new_agent = new_agent + np.random.rand(dim) * (chaotic_ES - 1)
                self.population[i] = new_agent

            # Ažuriranje GUI progres bara i vizuelizacija trenutnog napretka
            ...
            time.sleep(0.1)  # simulacija vremena za iteraciju
