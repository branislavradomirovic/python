...
def fitness_function_lstm(agent: np.ndarray, experiment_data: List[pd.DataFrame], target_metric: str, 
    ...
    try:
        # Kreiranje modela bez eksplicitnog Input sloja
        model = Sequential()
        model.add(LSTM(units=100, return_sequences=True, input_shape=(60, 1)))  # Input shape ovde
        model.add(LSTM(units=50))
        model.add(Dense(units=1))
        model.build(input_shape=(None, 60, 1))

        # Prikupljanje težina samo za slojeve sa parametrima
        layer_weights_shapes = [layer.get_weights() for layer in model.layers if layer.weights]
        ...
        for data in experiment_data:
            ...
            for segment in segments:
                ...
                # Rekonstrukcija težina iz agenta
                start = 0
                new_weights = []
                for layer_weights in layer_weights_shapes:
                    ...
                    for w_shape in layer_weights:
                        ...
                        start += size
                    new_weights.extend(layer_weights_arr)  # Koristimo extend umesto append
                
                # Postavljanje težina i trening
...
class VNS_DS_LSTM_GUI(tk.Tk):
    ...
    def _create_data_controls(self):
        ...
        self.save_results_button = tk.Button(self.results_frame, text="Sačuvaj rezultate kao sliku", command=self.save_results_image)
                ...
                with zipfile.ZipFile(self.zip_file_path, 'r') as zip_file:
                    ...
                    for folder_name in sorted(zip_file.namelist()):
                        ...
                        if len(parts) >= 1 and parts[-1].endswith('.txt'):
                            ...
                            if parent_folder not in self.data_dict:
    ...
    def run_vns_ds_thread(self):
        ...
        for experiment, experiment_data in experiments.items():
            ...
            if len(valid_folders) != len(folders):
                ...
                return
            experiment_data_list = [pd.concat(self.data_dict[f]) for f in valid_folders]
            print(f"Broj DataFrame-ova za {experiment}: {len(experiment_data_list)}")

            for data in experiment_data_list:
                print("Ažuriranje EEG plot-a")
                self.update_eeg_plot(data, valid_folders[0])  # Assuming valid_folders is not empty
            ...
            model.add(LSTM(units=50))
            model.add(Dense(units=1))
            model.build(input_shape=(None, 60, 1))  # Build the model to initialize weights
            dim = sum(np.prod(w.shape) for w in model.get_weights())
            print(f"Dimenzija LSTM modela: {dim}")

            try:
                print("Kreiranje VNS_DS_LSTM instance")
                # Remove vns_ds_lstm from lambda parameters
                vns_ds_lstm = VNS_DS_LSTM(dim, pop_size, max_iter, theta, 
                                        lambda agent: fitness_function_lstm(agent, experiment_data_list, metric, 
                                                                            self.lstm_train_progress_bar, self), 
                                        vns_params, fa_params, self.vns_progress_bar, self)
                print("Pokretanje VNS_DS_LSTM algoritma")
                best_agent, best_metric = vns_ds_lstm.run()
                if self.stop_flag:
                    break
                if best_metric is not None and best_metric != float('-inf'):
                    ...
                    self.global_progress_bar.update_idletasks()
                    ...
                    if completed_experiments > 0:
                        ...
                        self.elapsed_time_label.update_idletasks()
                        self.estimated_time_label.update_idletasks()
                else:
